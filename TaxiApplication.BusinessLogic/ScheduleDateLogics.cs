   ...
   public class ScheduleDateLogics
    {
        ...
        public bool AddDate(ScheduleDate scheduleDate)
        {
            ...
            var countMax = db.ScheduleDates.Where(b => b.ExtendSchedule == "Yes").Select(b => b.Count).Max();


            var dateTo = db.ScheduleDates.Where(b => b.ExtendSchedule == "Yes" && b.Count == countMax).Select(b => b.DateTo).FirstOrDefault();
